...
we divide the data per quartiles of one of them and make 2D heatmaps of the resulting subsets. Execute these scripts by e.g. typing on terminal 
`> python3 heatmap_FC.py`.

The campaigns meant to provide the data for this analysis have to be excuted using the `RandomSampler` of the EasyVVUQ library.

### Sobol indices
The first order Sobol indices are computed by the script `sobol_indices.py`. By selecting different campaign states, the Sobol indices of all 
