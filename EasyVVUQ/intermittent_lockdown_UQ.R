...
lock_length <- unname(sapply(json_data$lockdown_length, as.numeric))
lock_length <- sapply(10 + 30*lock_length, as.integer)
...
intermittent_lock_data[, "IC_prev_avg_max"] <- max(IC_prev_avg)
