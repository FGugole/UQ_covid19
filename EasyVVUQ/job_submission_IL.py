...
campaign = uq.Campaign(name='virsim_IL_nobio_', work_dir=workdir)
...
# Save the campaign
campaign.save_state('campaign_state_IL_nobio.json')

# Run execution sequentially 
#campaign.apply_for_each_run_dir(uq.actions.ExecuteLocal('intermittent_lockdown_UQ.r corona_in.json', interpret='Rscript'))

# Run execution in parallel without Fabsim (using gnu parallel)
cwd = os.getcwd()
pcmd = f"ls -d {campaign.campaign_dir}/runs/Run_* | parallel -j 4 'cd {{}} ; Rscript {cwd}/intermittent_lockdown_UQ.r corona_in.json > output.txt ; cd .. '"
