...
vary = {
    ...
    "lockdown_length": cp.Gamma(shape=30, scale=3),
    "lift_length": cp.Gamma(shape=12, scale=3/4),
...
cwd = os.getcwd()
pcmd = f"ls -d {my_campaign.campaign_dir}/runs/Run_* | parallel -j 8 'cd {{}} ; Rscript {cwd}/intermittent_lockdown_UQ_bio.r corona_in.json > output.txt ; cd .. '"
print('Parallel run command: ',pcmd)
subprocess.call(pcmd,shell=True)

#Save the Campaign
my_campaign.save_state("campaign_state_IL.json")
