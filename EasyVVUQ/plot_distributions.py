#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Created on Thu Jul  2 09:41:32 2020

@author: federica
"""

import numpy as np
import chaospy as cp 
import matplotlib.pyplot as plt
plt.rcParams.update({'font.size': 20})
plt.rcParams['figure.figsize'] = 8,6

# Contact tracing
beta_tpE = cp.Beta(alpha=2, beta=6)
beta_tcr = cp.Beta(alpha=10, beta=2)
gamma_trI = cp.Gamma(shape=2, scale=.2)

x_CT = np.linspace(0, 1.5, 151)

f = plt.figure('distributions_CT')
ax = f.add_subplot(111, xlabel='x')#, ylabel='pdf')
ax.plot(x_CT,beta_tpE.pdf(x_CT),lw=2,label='Trace probability')
ax.plot(x_CT,gamma_trI.pdf(x_CT),lw=2,label='Rate of tracing')
ax.plot(x_CT,beta_tcr.pdf(x_CT),lw=2,label='Contact reduction')
ax.set_xticks([0.0, .5, 1.0, 1.5])
ax.set_ylim([0, 5])
ax.set_yticks([0, 1.5, 3, 4.5])

leg = plt.legend()
leg.set_draggable(True)
plt.tight_layout()

# Flattening the curve
beta_int1 = cp.Beta(alpha=38, beta=70)
beta_up = cp.Beta(alpha=16, beta=2)

x_FT = np.linspace(0, 1, 101)

f = plt.figure('distributions_FC')
ax = f.add_subplot(111, xlabel='x', ylabel='P(x)')
ax.plot(x_FT,beta_int1.pdf(x_FT),lw=2,label='Intervention effect')
ax.plot(x_FT,beta_up.pdf(x_FT),lw=2,label='Uptake')
ax.set_xticks([0.0, .5, 1.0])
ax.set_ylim([0, 9])
ax.set_yticks([0, 4, 8])

leg = plt.legend()
leg.set_draggable(True)
plt.tight_layout()

# Intermittent lockdown & Phased Opening
beta_lockeffect = cp.Beta(alpha=14,beta=42)
gamma_locklength = cp.Gamma(shape=20,scale=2)
gamma_liftlength = cp.Gamma(shape=15,scale=1)
gamma_phase_interval = cp.Gamma(shape=25,scale=2)

x_IL = np.linspace(0, 1, 101)
x_IL_length = np.linspace(0, 90, 901)

f = plt.figure('distributions_IL')
ax = f.add_subplot(111, xlabel='x', ylabel='P(x)')
ax.plot(x_IL,beta_lockeffect.pdf(x_IL),lw=2,label='Lockdown effect')
ax.plot(x_IL,beta_up.pdf(x_IL),lw=2,label='Uptake')
#ax.plot(x_IL,beta_lockeffect.pdf(x_IL),ls='--',lw=2,label='Relative level of transmission')
ax.set_xticks([0.0, .5, 1.0])
ax.set_ylim([0.0, 10.0])
ax.set_yticks([0, 4, 8])

leg = plt.legend()
leg.set_draggable(True)
plt.tight_layout()

f = plt.figure('distributions_IL_lengths')
ax = f.add_subplot(111, xlabel='x')#, ylabel='pdf')
ax.plot(x_IL_length,gamma_locklength.pdf(x_IL_length),lw=2,label='Lockdown period')
ax.plot(x_IL_length,gamma_liftlength.pdf(x_IL_length),lw=2,label='Lift period')
ax.plot(x_IL_length,gamma_phase_interval.pdf(x_IL_length),lw=2,label='Intervention interval')
ax.set_xticks([0, 30, 60, 90])
ax.set_ylim([0, 0.11])
ax.set_yticks([0.0, .1])

leg = plt.legend()
leg.set_draggable(True)
plt.tight_layout()

# Phased opening

x_PO = np.linspace(0, 1, 101)
x_PO_length = np.linspace(0, 90, 901)

f = plt.figure('distributions_PO')
ax = f.add_subplot(111, xlabel='x', ylabel='pdf')
ax.plot(x_PO,beta_lockeffect.pdf(x_PO),lw=2,label='pl_intervention_effect_hi')
ax.plot(x_PO,beta_up.pdf(x_PO),lw=2,label='uptake')
ax.set_xticks([0.0, .5, 1.0])
ax.set_ylim([0.0, 8.5])
ax.set_yticks([0, 4, 8])

leg = plt.legend()
leg.set_draggable(True)
plt.tight_layout()

f = plt.figure('distributions_PO_lengths')
ax = f.add_subplot(111, xlabel='x', ylabel='pdf')
ax.plot(x_PO_length,gamma_phase_interval.pdf(x_PO_length),lw=2,label='intervention_lift_interval')
ax.set_xticks([0, 30, 60, 90])
ax.set_ylim([0.0, 0.05])
ax.set_yticks([0.0, .02, .04])

leg = plt.legend()
leg.set_draggable(True)
plt.tight_layout()

# Biology related parameters
gamma_Rzero = cp.Gamma(shape=100,scale=.025)
gamma_duration_infectiousness = cp.Gamma(shape=25,scale=.2)
gamma_intervention_effect_var = cp.Gamma(shape=2,scale=.05)
gamma_exposed_time = cp.Gamma(shape=17.5,scale=1)

x_bio = np.linspace(0, 8, 801)
x_exposed_time = np.linspace(0, 40, 4001)

f = plt.figure('distributions_bio')
ax = f.add_subplot(111, xlabel='x', ylabel='P(x)')
ax.plot(x_bio,gamma_Rzero.pdf(x_bio),lw=2,label='$R_{0}$')
ax.plot(x_bio,gamma_duration_infectiousness.pdf(x_bio),lw=2,label='Duration of infectiousness')
ax.plot(x_bio,gamma_intervention_effect_var.pdf(x_bio),lw=2,label='intervention_effect_var$^{-1}$')
ax.set_xticks([0, 4, 8])
ax.set_ylim([0, 8])
ax.set_yticks([0, 4, 8])

leg = plt.legend()
leg.set_draggable(True)
plt.tight_layout()

f = plt.figure('distribution_exposed_time')
ax = f.add_subplot(111, xlabel='x')#, ylabel='pdf')
ax.plot(x_exposed_time,gamma_exposed_time.pdf(x_exposed_time),lw=2,label='shape param. of exposed time distr.')
ax.set_xticks([0, 20, 40])
ax.set_ylim([0.0, 0.12])
ax.set_yticks([0.0, .1])

leg = plt.legend()
leg.set_draggable(True)
plt.tight_layout()

plt.show()

### END OF CODE ###
