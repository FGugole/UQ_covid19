...
% you need to provide:
% * R-file that calls Virsim, e.g. flattening_the_curve_UQ.r
% * same R-file but then with uncertain parameters replaced by <X0001> etc,
%   e.g. flattening_the_curve_UQ.r.tpl
...
% outputs are stored in the folder specified below in ModelOpts.Archiving.FolderName, e.g. runs/
% the script plot_output.m can be used to load the full output files (not only QoI) and
...
% R filename
filename_R     = 'flattening_the_curve_UQ.r'; % pointing to R file in current directory
...
virsim_output  = 'output_flattening_the_curve.csv';

%% input uncertainties

% load case-specific parameters
% run(input_file);
InputOpts.Marginals(1).Name = 'Intervention';
InputOpts.Marginals(1).Type = 'Beta';
InputOpts.Marginals(1).Parameters = [38 70];

InputOpts.Marginals(2).Name = 'Uptake';
InputOpts.Marginals(2).Type = 'Beta';
InputOpts.Marginals(2).Parameters = [16 2];

InputOpts.Marginals(3).Name = 'Seed';
InputOpts.Marginals(3).Type = 'Uniform';
InputOpts.Marginals(3).Parameters = [2^14 2^16];
...
ModelOpts.Archiving.FolderName = 'runs_FC_MC100';
