...
# biology
Rzero <- 2.5 #unname(sapply(json_data$Rzero, as.numeric))

avg_duration_infectiousness <- 5.0 # unname(sapply(json_data$duration_infectiousness, as.numeric))

avg_contact_rate <- Rzero / avg_duration_infectiousness

shape_exposed_time <- 20.0 # unname(sapply(json_data$shape_exposed_time, as.numeric))

# intervention_effect_var_inv <- 0.1 # unname(sapply(json_data$intervention_effect_var_inv, as.numeric))

# strategy
lock_effect <- <X0001>

phase_interval <- <X0002>
...
#############################
write.csv(x=phased_opening_data, file='output_phased_opening.csv', row.names=FALSE)
