...
if (any(strcmp(methods,'PCE_OLS')))
    PCE = true;
    myPCE = myPCE_OLS;
    mean_PCE = mean_OLS;    
    std_PCE  = std_OLS;    
end
if (any(strcmp(methods,'PCE_LARS')))
    PCE = true;
    myPCE = myPCE_LARS;
    mean_PCE = std_LARS;        
end
...
if (PCE)
    ...
    % in case of multiple PCEs, this will be the last one
    N_PP = 1e5;
    X_PP = uq_getSample(N_PP,'MC');
    
    Y_PP = uq_evalModel(myPCE,X_PP);
    ...
    hold on
    plot([mean_PCE(end);mean_PCE(end)],ylim,'k-')
    plot([mean_PCE(end)+std_PCE(end);mean_PCE(end)+std_PCE(end)],ylim,'k--')
    plot([mean_PCE(end)-std_PCE(end);mean_PCE(end)-std_PCE(end)],ylim,'k--')
...
ylabel('Maximum of moving average of infected cases (mean)');
...
xlabel('N') % Add proper labelling and a legend
ylabel('Maximum of moving average of infected cases (std)');
grid on;
